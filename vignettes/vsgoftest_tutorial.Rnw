...
By default, \code{vs.test} performs the composite VS test of the family of distributions \code{densfun} for the sample \code{x}. The p-value is estimated by means of Monte-Carlo simulations if the sample size is smaller than 80, or through the asymptotic distribution~(\ref{asymptoticn}) of the VS test statistic otherwise.
...
For performing a simple null hypothesis GOF test, the additional argument \code{param} has to be set to a numeric vector, consistent with the parameter requirements for the null distribution.
...
One can estimate the p-value of the sample by Monte-Carlo simulations, even when sample size is larger than 80, by setting the optional argument \code{simulate.p.value} to \code{TRUE} (\code{NULL} by default). The number of Monte-Carlo replicates can be fixed through  the optional argument~\code{B} (default is \code{B = 5000}).
...
Note that upper-bounding the window size by $n^{1/3-\delta}$ is only required when the asymptotic normality of $I_{mn}$ is used to compute asymptotic p-values from~(\ref{asymptoticn}). When the p-value are computed by means of Monte-Carlo simulation, this upper-bound can be extended to $n/2$ by adding \code{extend = TRUE}, which may lead to a more reliable test, as illustrated below.
...
The following code chunks intend to illustrate the use and behaviour of the function \code{vs.test} for these environmental data. The significant level is fixed to $0.1$ as in \cite{singh}. 
